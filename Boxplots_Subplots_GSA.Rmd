...
df$Site <- factor(df$Site,
                    levels=c("US", "DS","Soil","Spring","Rain"))

# Wrapping some old code in an unused function:
# .f = function() {}

conservative <- df %>%
  select(ID, Site, Source, O18, D, SO4, Cl) %>%
  gather(Species, Concentration, O18:Cl)
...
redox <- df %>%
  select(ID, Site, Source, ORP, H2S_f, Fe, Mn) %>%
  gather(Species, Concentration, ORP:Mn)
...
conservative_plot <- ggplot(conservative, aes(x=Site, y=Concentration, fill=Site)) + 
  ...
  theme_bw() +
  scale_fill_manual(values = c(usc, dsc, soilc, springc, rainc))
...
ggsave(file = "conservative.pdf", plot = conservative_plot, width = 6, height = 3, dpi = 600)
ggsave(file = "bases.pdf", plot = bases_plot, width = 6, height = 3, dpi = 600)
ggsave(file = "nutrients_plot.pdf", plot = nutrients_plot, width = 10, height = 3, dpi = 600)
ggsave(file = "redox_plot.pdf", plot = redox_plot, width = 6, height = 3, dpi = 600)
```
