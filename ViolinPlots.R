...
violinplot_fun = function(x,y) {
  ggplot(df, aes(x = .data[[x]], y= .data[[y]])) +
    geom_violin(aes(colour = .data[[y]]), fill = "gray", trim=FALSE) +
    geom_boxplot(aes(colour = .data[[y]]), width=0.1, fill="white") +
    geom_jitter(aes(colour = .data[[y]]), shape=16, position=position_jitter(0.05), alpha=0.5) +
    theme_classic() +
...
all_plots = map(response,
                ~map(exp1, violinplot_fun, y = .x))

iwalk(all_plots, ~{
  pdf(paste0(.y, "_violinplots.pdf") )
  print(.x)
  dev.off()
})
